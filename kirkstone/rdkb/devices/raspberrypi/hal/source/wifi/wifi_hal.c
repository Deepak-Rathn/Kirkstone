...
{
   FILE * fp = popen("cat /proc/uptime | cut -d ' ' -f1", "r");
   if ( !fp )
       return -1;
   fscanf(fp,"%lu",time);
   pclose(fp);
...
static int rpi_hostapdProcessUpdate(int apIndex, struct hostapd_params *list, int item_count)
{
    ...
    {
        snprintf(cmd, sizeof(cmd), "hostapd_cli -i%s SET %s %s",ifname, list->name, list->value);
...
static int rpi_reloadAp(int apIndex)
{
    ...
    Dynamically_Disabling_hostapd_process(apIndex,TRUE);
...
{
	char cmd[128];
	/* check  /usr/ccsp/wifi/hostapd0.conf, /usr/ccsp/wifi/hostapd1.conf , /usr/ccsp/wifi/hostapd4.conf,/usr/ccsp/wifi/hostapd5.conf exists or not */
	...
	if(WEXITSTATUS(system("lsmod | grep mt76*")) == 0)
	{
		printf("Netgear module is detected so hostapd configuration preparation is handled by hostapd boot up script.. \n");
	}	
	else
	{	
...
static struct ctrl wpa_ctrl[16];
static int initialized;
static pthread_t event_monitor_tid;
...
void *event_monitor_thread(void)
{
        while(1)
        {
...
static int init_wpa()
{
        ...
        for (s = 0; s < snum; s++) {
                ...
                GetInterfaceName_HostapdConf(s,ifname);
                if(!strcmp(ifname,""))
                      sprintf(ifname,"wifi%d",s);
                sprintf(wpa_ctrl[s].sockpath, "%s%s", SOCK_PREFIX, ifname);
        ...
        initialized = 1;
        pthread_create(&event_monitor_tid, NULL, event_monitor_thread, NULL);
...
//knowning the current status of radio and ssid
void get_radio_ssid_currentstatus(int index,char *ssid_status, char *radio_status)
{
        ...
        if(index == 0)
        {
                ...
                fp1 = fopen("/var/Get2gRadioEnable.txt","r");
        }
        else
        {
                fp = fopen("/var/Get5gssidEnable.txt","r");
                fp1 = fopen("/var/Get5gRadioEnable.txt","r");
        }
        if((fp == NULL) || (fp1 == NULL))
                return RETURN_OK;
        if(fgets(path, sizeof(path)-1, fp) != NULL)
        {
                for(count=0;path[count]!='\n';count++)
                        ssid_status[count]=path[count];
                ssid_status[count]='\0';
        }
        fclose(fp);
        memset(path,0,sizeof(path));
        if(fgets(path, sizeof(path)-1, fp1) != NULL)
        {
                for(count=0;path[count]!='\n';count++)
                        radio_status[count]=path[count];
                radio_status[count]='\0';
        }
        fclose(fp1);
	return RETURN_OK;
}
...
void Stop_Start_Broadcasting_SSID_Names_Hostapd_Process(int index,BOOL *Public_ssid,char *Alias_Interface_name)
{
        char cmd[MAX_CMD_SIZE] = {0};
        if(*Public_ssid == 0)
                sprintf(cmd,"hostapd_cli -i %s SET ignore_broadcast_ssid 1",Alias_Interface_name);
        else
                sprintf(cmd,"hostapd_cli -i %s SET ignore_broadcast_ssid 0",Alias_Interface_name);
        system(cmd);
}
void get_pssid_currentstatus(int apIndex,char *ifname)
{

        FILE *fp = NULL;
        int count;
        char path[32] = {0},buf[64] = {0},pssid_status[10] = {0};
        if(apIndex == 4)
        ...
        {
                for(count=0;path[count]!='\n';count++)
                        pssid_status[count]=path[count];
                pssid_status[count]='\0';
        }
        fclose(fp);
	if(!strcmp(pssid_status,"0"))
	{
		snprintf(buf,sizeof(buf),"hostapd_cli -i%s disable",ifname);
		system(buf);
	}
	return RETURN_OK;
}
...
void Dynamically_Disabling_hostapd_process(int apIndex,BOOL flag)
{
	char radio_status[10] = {0},ssid_status[10] = {0};
	char interface_name[10] = {0};
	char cmd[MAX_CMD_SIZE] = {0};
	GetInterfaceName_HostapdConf(apIndex,interface_name);
	get_radio_ssid_currentstatus(apIndex,ssid_status,radio_status);
	if((apIndex == 4) || (apIndex ==5))
		get_pssid_currentstatus(apIndex,interface_name);
	if((strcmp(ssid_status,"0") == 0 ) || (strcmp(radio_status,"0") == 0) || (flag == TRUE))			
	{
...
void Dynamically_Enabling_hostapd_process(int apIndex)
{
	...
	get_radio_ssid_currentstatus(apIndex,ssid_status,radio_status);
        ...
        if((strcmp(radio_status,"1") == 0 ) && (strcmp(ssid_status,"1") == 0))
        {
...
void Dynamically_Updated_SupportedRated_hostapd_process(int apIndex, char *string)
{
        ...
        char cmd[MAX_CMD_SIZE] = {0};
        GetInterfaceName_HostapdConf(apIndex,interface_name);
        sprintf(cmd,"hostapd_cli -i %s SET supported_rates \"%s\"",interface_name,string);
        system(cmd);
        Dynamically_Disabling_hostapd_process(apIndex,TRUE);
        Dynamically_Enabling_hostapd_process(apIndex);
...
void DYnamically_Updated_OperatingChannelBandwidth(int radioIndex,char *String)
{
	...
	sprintf(cmd,"hostapd_cli -i %s SET vht_oper_chwidth  %s",interface_name,String);
	system(cmd);
        Dynamically_Disabling_hostapd_process(radioIndex,TRUE);
...
void Dynamically_Updated_Channel_Value_hostapd_process(INT apIndex,ULONG Value)
{
	...
	if(apIndex == 0)
		...
		if(strcmp(xfinity_wifi,"1") == 0)
		{
                ...
                Dynamically_Disabling_hostapd_process(4,TRUE);
		...
		if(strcmp(xfinity_wifi,"1") == 0)
		{
                ...
                Dynamically_Disabling_hostapd_process(5,TRUE);
...
INT wifi_factoryReset()
{
	...
	wifi_dbg_printf("\n[%s]: deleting hostapd conf file %s and %s\n",__func__,HOSTAPD_CONF_0,HOSTAPD_CONF_1);
	sprintf(cmd, "rm -rf %s %s",HOSTAPD_CONF_0,HOSTAPD_CONF_1);
	system(cmd);
	system("rm -rf /var/Get*");
...
INT wifi_reset()
{
	...
	for(ap = 0; ap < WIFI_HAL_TOTAL_NO_OF_APS; ap++)   
	{
		...
		{		  
			Dynamically_Disabling_hostapd_process(ap,TRUE);
...
INT wifi_setRadioEnable(INT radioIndex, BOOL enable)            //RDKB
{
	...
	if(radioIndex == 0)
	{
		...
		if(enable == FALSE)
		{
			Dynamically_Disabling_hostapd_process(0,FALSE);
			if(strcmp(xfinity_wifi,"1") == 0)
			{
	...
	else
	{
		...
		if(enable == FALSE)
		{
			Dynamically_Disabling_hostapd_process(1,FALSE);
			if(strcmp(xfinity_wifi,"1") == 0)
				Dynamically_Disabling_hostapd_process(5,FALSE);
		}
		else
		{
			Dynamically_Disabling_hostapd_process(1,FALSE);
...
INT wifi_setRadioChannel(INT radioIndex, ULONG channel)	//RDKB	//AP only
{
  ...
  if(radioIndex == 0)
  {
  	wifi_hostapdWrite(0,&list);
  	wifi_hostapdWrite(4,&list);
  }
  else
  {
  	wifi_hostapdWrite(1,&list);
...
{
        WIFI_ENTRY_EXIT_DEBUG("Inside %s:%d\n",__func__, __LINE__);
	POINTER_CHECK(output_string != NULL);
        radioIndex_Check(radioIndex);
#if defined(_ONE_WIFI_)
	char buf[128] = {0};
        snprintf(buf,sizeof(buf),"iw dev wlan%d info | grep width | cut -d ':' -f2 | cut -d ',' -f1 | xargs",radioIndex);
	memset(output_string, 0, sizeof(output_string));
	if(!_syscmd(buf, output_string, sizeof(output_string)))
        {
               if ( output_string[0] == '\n' || ( output_string == NULL ) || ( output_string[0] == '\0'))
               {
                    wifi_dbg_printf("\n[%s]Radio %s bandwidth value : .Returning error as received empty values\n",__func__,output_string);
                    return 1;
               }
               else
               {
                    strcat(output_string,"MHz");
                    wifi_dbg_printf("\n[%s]Radio %s bandwidth value : \n",__func__,output_string);
                    return 0;
               }
...
{
        WIFI_ENTRY_EXIT_DEBUG("Inside %s:%d : %s\n",__func__, __LINE__,output_string);
  	struct params params={'\0'};
 	param_list_t list;
	char bw_value[10] = {0};
	char value[15] = {0};
	...
	else if(strcmp(output_string,"8080MHz") == 0)
	{
		output_string="3";
		writeBandWidth(radioIndex,"8080MHz");
	}
	else
	{
...
INT wifi_getRadioTransmitPower(INT radioIndex, ULONG *output_ulong)	//RDKB
{
	...
	snprintf(cmd, sizeof(cmd),  "iw wlan%d info | grep txpower | cut -d ' ' -f2", apIndex);
	_syscmd(cmd, buf, sizeof(buf));
...
INT wifi_getRadioTrafficStats2(INT radioIndex, wifi_radioTrafficStats2_t *output_struct) //Tr181
{
	...
	{
#if defined(_ONE_WIFI_)
		strcpy(private_interface_name, "wlan0");
#else
	...
	{
#if defined(_ONE_WIFI_)
		strcpy(private_interface_name,"wlan1");
#else
...
{ 
       return RETURN_OK; //this api is not used 
}
//<<
...
{
         char buf[126]={'\0'},interface_name[32];
	 GetInterfaceName_HostapdConf(apIndex,interface_name);
...
INT wifi_getApDevicesAssociated(INT apIndex, CHAR *macArray, UINT buf_size) 
{
       ...
       char mac[258]= {'\0'};
       char buf[258]= {'\0'};
       char tmp[258]= {'\0'},interface_name[32];
       char *token=NULL;
       GetInterfaceName_HostapdConf(apIndex,interface_name);
       sprintf(buf,"hostapd_cli -i%s all_sta | grep dot11RSNAStatsSTAAddress",interface_name);
       fp=popen(buf,"r");
		...
		else
		{
			if(strcmp(xfinity_wifi,"1") == 0)
			{
				if(apIndex == 4)
				{
					...
					system(command);
					Dynamically_Disabling_hostapd_process(4,FALSE);
					Dynamically_Enabling_hostapd_process(4);
					system("brctl addif brlan2 gretap0.102");
				}
				else if(apIndex == 5)
...
INT wifi_getApStatus(INT apIndex, CHAR *output_string)
{
       WIFI_ENTRY_EXIT_DEBUG("Inside %s:%d\n",__func__, __LINE__);
        ...
        {
               wifi_getSSIDEnable(apIndex, &output_bool);
               if ( output_bool == 1)
               {
                       if ((apIndex == 0) || (apIndex == 4))
                       {
                               wifi_getRadioEnable(0, &output_bool);
                       }
                       else
                       {
                               wifi_getRadioEnable(1, &output_bool);
                       }
                       if(output_bool == 1)
                       {
...
INT wifi_getApWpsEnable(INT apIndex, BOOL *output_bool)
{
	...
	if((apIndex == 0 ) || (apIndex == 1))
        ...
        {
                *output_bool=FALSE;
                 printf("This index is not supported \n");
                 return RETURN_ERR; 
        } 
	_syscmd(cmd,buf, sizeof(buf));
	if(strlen(buf)>0)
...
INT wifi_getApWpsConfigurationState(INT apIndex, CHAR *output_string)
{
        WIFI_ENTRY_EXIT_DEBUG("Inside %s:%d\n",__func__, __LINE__);
	char cmd[64];
	char buf[512]={0},interface_name[32];
	char *pos=NULL;
...
{
	char cmd[64];
	char buf[256]={0},interface_name[32];
	BOOL enable;
        GetInterfaceName_HostapdConf(apIndex,interface_name);
	if((apIndex == 0) || (apIndex == 1))
	{
...
{
	INT ret = RETURN_OK;
	POINTER_CHECK(output_array_size != NULL);
	POINTER_CHECK(associated_dev_array != NULL);
	apIndex_Check(apIndex);
...
INT wifi_getRadioUpTime(INT radioIndex, ULONG *output)
{
    ...
    char buf[MAX_BUF_SIZE] = {0};
    char cmd[MAX_CMD_SIZE] = {0},interface_name[10];
    ULONG radio_uptime = 0,current_time = 0;
    status = wifi_getRadioEnable(radioIndex, &output_bool);  //check radio
    if((status == RETURN_ERR) || (output_bool == FALSE))
    {
        *output = 0;
        return RETURN_OK;
    }
    GetInterfaceName_HostapdConf(radioIndex,interface_name);
    if (radioIndex == 0)
        snprintf(cmd,sizeof(cmd),"dmesg | grep \"link becomes ready\" | grep %s | tail -1 | cut -d ']' -f1 | cut -d '.' -f1 | cut -d '[' -f 2 | sed \"s/^ *//g\"",interface_name);
    else
        snprintf(cmd,sizeof(cmd),"dmesg | grep \"entered\" | grep %s | tail -1 | cut -d ']' -f1 | cut -d '.' -f1 | cut -d '[' -f 2 | sed \"s/^ *//g\"",interface_name);
    status = _syscmd(cmd, buf, sizeof(buf));
    if ((status != 0) && (strlen(buf) == 0))
        return RETURN_ERR;
    radio_uptime=atol(buf);
    if (getCurrenttime(&current_time) == -1)
        return RETURN_ERR;
    *output = current_time - radio_uptime;
    return RETURN_OK;
}
...
{
	INT ret = RETURN_OK;
	POINTER_CHECK(mac_addr != NULL);
	POINTER_CHECK(dev_conn != NULL);
	apIndex_Check(apIndex);
	ret = rpi_getApAssociatedClientDiagnosticResult(apIndex, mac_addr,dev_conn);
	if(ret == RETURN_ERR)
		printf("func[%s], apIndex[%d] ret[%d] associated_dev_array[%p] \n",
				__FUNCTION__, apIndex, ret, *dev_conn);
	WIFI_ENTRY_EXIT_DEBUG("Exiting %s:%d\n",__func__, __LINE__);
	return RETURN_OK;
}
...
{
    char cmd[256],interface_name[10];
    GetInterfaceName_HostapdConf(apIndex,interface_name);  	
...
INT wifi_pushMultiPskKeys(INT apIndex, wifi_key_multi_psk_t *keys, INT keysNumber)
{
    FILE *fd      = NULL;
    char fname[100];
    char cmd[128] = {0},interface_name[10];
    char out[64] = {0};
		...
		wifi_NeighborReport_t *neighborReports)
{
    ...
    char nr[256] = { 0 };
    char ssid[256];
    char hex_ssid[256],interface_name[10];
    INT ret;
...
int main(int argc,char **argv)
{
    ...
    if(strstr(argv[1], "getApEnable")!=NULL) {
        ...
        printf("%s %d: %s, returns %d\n", argv[1], index, status, ret);
    ...
    else if(strstr(argv[1], "wifi_kickAssociatedDevice")!=NULL) {
            char mac[64] = {'\0'};
            char *token = NULL;
            wifi_device_t dev = {0};
            int count = 0;
            strncpy(mac, argv[3], 64);
            token = mac;
            ...
            printf("MAC Address : %02x:%02x:%02x:%02x:%02x:%02x\n", dev.wifi_devMacAddress[0], dev.wifi_devMacAddress[1],
                    dev.wifi_devMacAddress[2], dev.wifi_devMacAddress[3], dev.wifi_devMacAddress[4], dev.wifi_devMacAddress[5],
                    dev.wifi_devAssociatedDeviceAuthentiationState);
            ret = wifi_kickAssociatedDevice(index, &dev);
	...
	else if(strstr(argv[1], "wifi_getApAssociatedClientDiagnosticResult")!=NULL) {
                wifi_associated_dev3_t *associated_dev_array=NULL, *pt=NULL;
                char cli_mac[32] = {0};
		strcpy(cli_mac,argv[3]);
		int array_size =1,i;
                ret=wifi_getApAssociatedClientDiagnosticResult(index, &cli_mac, &associated_dev_array);
                printf("%s %d: returns %d\n", argv[1], index,  ret);
                for(i=0, pt=associated_dev_array; i<array_size; i++, pt++) {
                        printf("  associated_dev %d:\n", i);
                        printf("     cli_OperatingStandard      =%s\n", pt->cli_OperatingStandard);
