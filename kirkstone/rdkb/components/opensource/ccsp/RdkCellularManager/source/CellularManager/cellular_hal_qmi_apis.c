...
typedef enum {
    ...
    MODEM_OPEN_STATE_NAS_CELL_LOCATION_INFO,
...
typedef struct 
{
    ...
    guint32                         globalCellId;
    guint                           bandInfo;
    guint16                         servingCellId;
...
//Retrieve Eutra band info
guint eutra_band_info(guint16 earfcn)
{
    guint i;
...
static void  cellular_hal_qmi_device_open (QmiDevice *device, GAsyncResult *result, gpointer user_data)
{
    ...
    if (!qmi_device_set_expected_data_format(device, QMI_DEVICE_EXPECTED_DATA_FORMAT_RAW_IP, &error)) {
        CELLULAR_HAL_DBG_PRINT ("%s %d Failed to set RAW_IP : %s\n", __FUNCTION__, __LINE__, error->message);
        g_error_free (error);
        pDeviceOpenCtx->enOpenStatus  = DEVICE_OPEN_STATUS_NOT_READY;
        pDeviceOpenCtx->uiCurrentStep = MODEM_OPEN_STATE_NOTIFY;
        cellular_hal_qmi_device_open_step(task);
    ...
    //Verify whether wwan_ifacename available or not
    if (wwan_ifacename == NULL) {
        CELLULAR_HAL_DBG_PRINT ("%s %d QMI wwan iface is NULL\n", __FUNCTION__, __LINE__);
        pDeviceOpenCtx->enOpenStatus  = DEVICE_OPEN_STATUS_NOT_READY;
        pDeviceOpenCtx->uiCurrentStep = MODEM_OPEN_STATE_NOTIFY;
        cellular_hal_qmi_device_open_step(task);
        return;
                                                ...
                                                gpointer user_data )
{
    ...
    if (!output)
    { 
        CELLULAR_HAL_DBG_PRINT("%s cell location info get functionality failed: %s\n",__FUNCTION__,error->message);
        g_error_free (error);
        goto NEXTSTEP;
    }
    ...
    {
        CELLULAR_HAL_DBG_PRINT("%s failed to get cell location info: %s\n",__FUNCTION__,error->message);
        g_error_free (error);
    ...
    CELLULAR_HAL_DBG_PRINT("Intrafrequency LTE Info\n"
    ...
    nasCtx->servingCellId = serving_cell_id;  
...
static void cellular_hal_qmi_device_open_step( GTask *task )
{
    ...
    switch ( pDeviceOpenCtx->uiCurrentStep ) 
    {
        ...
        case MODEM_OPEN_STATE_NAS_CELL_LOCATION_INFO:
        {
            ContextNASInfo   *nasCtx = &(pstQMIContext->nasCtx);
        ...
        case MODEM_OPEN_STATE_NOTIFY:
        {
            ...
            //Verify whether QMI device is opened or not
            if ( pDeviceOpenCtx->enOpenStatus  == DEVICE_OPEN_STATUS_NOT_READY)
            {
                 CELLULAR_HAL_DBG_PRINT("%s %d - QMI device is not opened\n",__FUNCTION__,__LINE__);
                 //Needs to close the QMI device
                 qmi_device_close_async (gpstQMIContext->qmiDevice,
                            5,
                            NULL,
...
int cellular_hal_qmi_get_cell_location_info(CellLocationInfoStruct *loc_info)
{
    //Check whether QMI ready or not
    if( ( NULL != gpstQMIContext ) && \
        ( NULL != gpstQMIContext->qmiDevice ) && \
        ( TRUE == qmi_device_is_open( gpstQMIContext->qmiDevice ) ) )
    {
        ...
        if( NULL != nasCtx->nasClient )
        {
            memset( loc_info, 0, sizeof(CellLocationInfoStruct) );
            loc_info->globalCellId = nasCtx->globalCellId;
            loc_info->bandInfo = nasCtx->bandInfo;
            loc_info->servingCellId = nasCtx->servingCellId; 
        } 
        else
        {
            CELLULAR_HAL_DBG_PRINT("%s - NAS client is not ready \n", __FUNCTION__);
        }
    }
    else
    {
        CELLULAR_HAL_DBG_PRINT("%s - QMI Device is not ready  ..cannot populate any info now \n", __FUNCTION__);
        return RETURN_ERROR;
    }
    ...
    switch ( pProfileCrCtx->uiCurrentStep ) 
    {
        ...
        case WDS_PROFILE_MODIFY_VERIFY_PROFILE_PRESENT:
        {
                 ...
                 ( WDS_PROFILE_OPERATION_MODIFY == pProfileCrCtx->enProfileOperationRequest ) )
            {
                    ...
                    if ( NULL != pstInputProfile )
                    {
                        ...
                        for( i = 0; i < wdsCtx->ui8ProfileCount; i++ )
                        {
                                ...
                                ( pstProfileList[i].PDPAuthentication == pstInputProfile->PDPAuthentication ) )
                            {
                                ...
                                CELLULAR_HAL_DBG_PRINT("%s - Matched with existing profile(%d).\n",__FUNCTION__,pstProfileList[i].ProfileID);
                ...
                // Selecting the default profile for following 2 scenarios.
                // i)  If PSM stored profile didn't match with any of the modem provided profiles.
                // ii) If fetching modem profile list has failed even after 2 retries.

                // TODO: handle the below later.
                // profile status cb is used to differentiate the dmcli call flow vs state machine call flow.
                if( (FALSE == pProfileCrCtx->bIsProfileAlreadyPresent && NULL != wdsCtx->pstProfileList) ||
                    (pProfileCrCtx->device_profile_status_cb && getProfileRetry > 2) )
                {
                   //setting modem default profile
                    CELLULAR_HAL_DBG_PRINT("%s - No matching profile found!\n", __FUNCTION__);
                    getProfileRetry = 0;

                    if ( CELLULAR_PROFILE_ID_UNKNOWN != wdsCtx->stDefaultProfileFromModem.ProfileID )
                    {
                         //Copy Input Profile information Default Profile
                         memset(pstInputProfile, 0, sizeof(CellularProfileStruct));
                         memcpy(pstInputProfile, &(wdsCtx->stDefaultProfileFromModem), sizeof(CellularProfileStruct));
                         CELLULAR_HAL_DBG_PRINT("%s - Default profile(%d).\n",
                                                           __FUNCTION__, pstInputProfile->ProfileID);
                    }
                    else
                    {
                         CELLULAR_HAL_DBG_PRINT("%s - No modem default profile found!!\n", __FUNCTION__);
                    }
                }
        ...
        {
            if ( WDS_PROFILE_OPERATION_CREATE == pProfileCrCtx->enProfileOperationRequest &&
                 !pProfileCrCtx->device_profile_status_cb )
            {
                CellularProfileStruct    *pstInputProfile   = &(pProfileCrCtx->stProfileInput);
        ...
        case WDS_PROFILE_MODIFY_NOTIFY:
        {
                 ...
                 ( WDS_PROFILE_OPERATION_MODIFY == pProfileCrCtx->enProfileOperationRequest ) )
            {
                ...
                if( NULL != pProfileCrCtx->device_profile_status_cb )
                {
                    if( getProfileRetry )
                    {
                        CELLULAR_HAL_DBG_PRINT("%s - sending profile status not ready via CB\n", __FUNCTION__);
                        pProfileCrCtx->device_profile_status_cb( pstInputProfile->ProfileName, pstInputProfile->PDPType, DEVICE_PROFILE_STATUS_NOT_READY );
                    }
                    else
                    if( WDS_PROFILE_OPERATION_DELETE == pProfileCrCtx->enProfileOperationRequest )
                    {
                                                          ...
                                                          gpointer  user_data)
{
    ...
    if (qmi_message_wds_get_current_settings_output_get_domain_name_list (output, &array, NULL)) {
        GString *s = NULL;
