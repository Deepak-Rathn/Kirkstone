...
$(document).ready(function() {
    ...
    $("#restore-default-settings").click(function(e) {
        ...
        jConfirm(
            "The firewall security level is currently set to " + currentSetting + ". Are you sure you want the change Firewall Options and Security Level to default settings?"
            ,"Reset Default Firewall Settings"
            ,function(ret) {
                if(ret) {
                	$("#firewall_level_maximum").prop("checked",false);
                  $("#firewall_level_minimum").prop("checked",true);
                  $("#true_static_ip").prop("checked",false);
                  $("#wan_ping").prop("checked",false);

        					var firewallLevel = "Low";
                  var trueStaticIp = "Disabled";
                  var wanPing = "Disabled";
        					var firewallCfg = '{"firewallLevel": "' + firewallLevel + '", "true_static_ip": "' + trueStaticIp + '", "wan_ping": "' + wanPing + '"}';
    ...
    $('#submit_firewall').click(function(){
        ...
        var obj7 = document.getElementById('true_static_ip');
        if (obj7.checked) {
          trueStaticIp = "Enabled";
        }

        var obj8 = document.getElementById('wan_ping');
        if (obj8.checked) {
          wanPing = "Enabled";
        }
        ...
        var firewallCfg = '{"firewallLevel": "' + firewallLevel + '", "block_http": "' + blockHttp + '", "block_icmp": "' + blockIcmp +
                                 ...
                                 '", "true_static_ip": "' + trueStaticIp + '", "wan_ping": "' + wanPing + '"} ';
...
<div id="content">
    ...
    $firewall_param = array(
      ...
      "true_static_ip"    => "Device.X_CISCO_COM_Security.Firewall.TrueStaticIpEnable",
      "wan_ping"          => "Device.X_CISCO_COM_Security.Firewall.WanPingEnable",
    );
    ...
    $true_static_ip = $firewall_value["true_static_ip"];
    $wan_ping = $firewall_value["wan_ping"];
    ?>
		<h2>Firewall Options</h2>
  		<p><input type="checkbox" id="true_static_ip" name="true_static_ip" <?php if (!strcasecmp("true",$true_static_ip)) echo "checked"; ?> />Disable Firewall for True Static IP Subnet Only </p>
