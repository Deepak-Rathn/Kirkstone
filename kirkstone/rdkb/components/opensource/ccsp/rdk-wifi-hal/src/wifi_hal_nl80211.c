...
#if  (defined(TCXB7_PORT) || defined(CMXB7_PORT) || defined(TCXB8_PORT) || defined(XB10_PORT) || defined(TCHCBRV2_PORT))
    int phy_rate = 60;
#endif
...
#else
#if defined(RDK_ONEWIFI) && (defined(TCXB7_PORT) || defined(CMXB7_PORT) || defined(TCXB8_PORT) || defined(XB10_PORT) || defined(TCHCBRV2_PORT))
...
int wifi_drv_switch_channel(void *priv, struct csa_settings *settings)
{
    ...
    for (i = 0; i < csa_off_len; i++) {
        ...
        if (settings->beacon_csa.tail[csa_c_off_bcn] != settings->cs_count) {
            ...
            wifi_hal_error_print("%s:%d: beacon_csa.tail[csa_c_off_bcn]=%ld settings->cs_count=%d\n",
...
int wifi_hal_purgeScanResult(unsigned int vap_index, unsigned char *sta_mac)
{
    ...
    if (radio != NULL) {
        ...
        if ((interface != NULL) && (interface->scan_info_map != NULL)) {
            ...
            pthread_mutex_lock(&interface->scan_info_mutex);
            free(hash_map_remove(interface->scan_info_map, key));
            pthread_mutex_unlock(&interface->scan_info_mutex);
        } else {
            return RETURN_ERR;
        }
...
#endif
{
    ...
    nla_put_u8(msg, NL80211_ATTR_KEY_IDX, params->key_idx);
#if defined(TCXB7_PORT) || defined(TCXB8_PORT) || defined(XB10_PORT) || defined (TCHCBRV2_PORT) || defined(_PLATFORM_RASPBERRYPI_)
