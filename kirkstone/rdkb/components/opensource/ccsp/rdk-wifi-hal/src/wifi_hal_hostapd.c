...
int update_hostap_iface(wifi_interface_info_t *interface)
{
    ...
    static const int basic_rates_g[] = { 10, 20, 55, 110, -1 };
...
#ifdef FEATURE_IEEE80211BE
        /*
            TODO: Need to add a rule for 320MHz
        */
#endif
...
#ifdef FEATURE_IEEE80211BE    
    /*
        TODO: we need to check if some specific params should be set
        for other platforms.
    */
#endif
...
static void print_hw_variants_by_bitmask(uint32_t mask)
{
    ...
    for (size_t i = 0; i < COUNT_OF(wifi_mode_strings); i++) {
        if (mask & (1ul << i))
...
static void print_bw_variants_by_bitmask(uint32_t mask)
{
    ...
    for (size_t i = 0; i < COUNT_OF(wifi_bw_strings); i++) {
        if (mask & (1ul << i))
...
#ifdef FEATURE_IEEE80211BE
    bool is_ieee80211be = !(param->variant & ~WIFI_80211_VARIANT_BE) && (param->variant & WIFI_80211_VARIANT_BE);
     if (is_ieee80211be) {
        if (param->band == WIFI_FREQUENCY_5_BAND) {
            iconf->hw_mode = HOSTAPD_MODE_IEEE80211A;
            iconf->ieee80211ac = 1;
