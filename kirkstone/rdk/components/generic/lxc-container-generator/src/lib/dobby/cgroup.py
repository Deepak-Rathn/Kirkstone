...
class cCgroupDobby(cCgroup):
    ...
    def createCGroupConf(self, CGroupNode):
        ...
        if CGroupNode is not None:
            ...
            swappiness = CGroupNode.find("Swappiness")
            if self.sanityCheck.validateTextEntry(swappiness):
                merge(entry, {"linux": {"resources": {"memory": {"swappiness": int(swappiness.text)}}}})
