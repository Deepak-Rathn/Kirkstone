...
class cConfigDobby(cConfig):
    ...
    def createGamepadConfig(self, configNode):

        entry = {}

        GamepadNode = configNode.find("Gamepad")

        if GamepadNode is not None:
            enable = GamepadNode.attrib["enable"]
    ...
    def createLocalTimeConfig(self, configNode):
        ...
        if LocalTimeNode is not None:
            ...
            if enable.lower() == "true":
                ...
                if "path" in LocalTimeNode.attrib:
                    entry["rdkPlugins"]["localtime"]["data"]["path"] = LocalTimeNode.attrib["path"]
    ...
    def createLxcConf(self, configNode):
        ...
        print("[%s] Create Dobby Gamepad configuration" % (self.sanityCheck.getName()))
        merge(data, self.createGamepadConfig(configNode))
