...
downloadBlacklist()
{
    ...
    if [[ ! -f $BLACKLIST_PATH ]] || [[ $(expr $(date +%s) - $(stat -c %Y $BLACKLIST_PATH)) -ge 86400 ]]; then
        ...
        if [ "$DEVICE_TYPE" = "broadband" ]; then
            ...
            if [ -f $EnableOCSPStapling ] || [ -f $EnableOCSP ]; then
                curl --cert-status --connect-timeout 10 --max-time 30 -w '%{http_code} %{remote_ip} %{remote_port}' https://${s3bucketurl}/ccp-stbcrashes/BLACKLISTS/blacklist.txt -o $BLACKLIST_PATH > $HTTP_CODE
            else
                curl --connect-timeout 10 --max-time 30 -w '%{http_code} %{remote_ip} %{remote_port}' https://${s3bucketurl}/ccp-stbcrashes/BLACKLISTS/blacklist.txt -o $BLACKLIST_PATH > $HTTP_CODE
            fi
            TLSRet=$?
            FQDN=`echo "$s3bucketurl"|awk -F/ '{print $3}'`
...
{    
    ...
    35|51|53|54|58|59|60|64|66|77|80|82|83|90|91)
        ...
        t2ValNotify "certerr_split" "DumpUL, $TLSRet, $FQDN"
    ;;
    esac
}
...
uploadToS3()
{
	...
	if [ "$DEVICE_TYPE" = "broadband" ]; then
              local ec=` exec_curl_mtls "$CURL_ARGS"`
