...
#Check for minidump and coredump files
if [ ! -e $MINIDUMPS_PATH/*.dmp* -a ! -e $CORE_PATH/*_core*.* -a ! -e $CORE_PATH/*.dmp ]; then exit 0; fi
...
downloadBlacklist()
{
    ...
    if [[ ! -f /opt/blacklist.txt ]] || [[ $(expr $(date +%s) - $(stat -c %Y /opt/blacklist.txt)) -ge 86400 ]]; then
        ...
        if [ "$DEVICE_TYPE" = "broadband" ]; then
            ...
            TLSRet=$?
            FQDN=`echo "$s3bucketurl"|awk -F/ '{print $3}'`
            logMessage "Curl return code : $TLSRet"
        else
            if [ -f $EnableOCSPStapling ] || [ -f $EnableOCSP ]; then
                curl --cert-status --connect-timeout 10 --max-time 30 -w '${CURL_LOG_OPTION}' https://${s3bucketurl}/ccp-stbcrashes/BLACKLISTS/blacklist.txt -o /opt/blacklist.txt > $HTTP_CODE
            else
                curl --connect-timeout 10 --max-time 30 -w '${CURL_LOG_OPTION}' https://${s3bucketurl}/ccp-stbcrashes/BLACKLISTS/blacklist.txt -o /opt/blacklist.txt > $HTTP_CODE
            fi
            TLSRet=$?
            FQDN=`echo "$s3bucketurl"|awk -F/ '{print $3}'`
            ...
            logMessage "Curl return code : $TLSRet, http code: $http_code"
        fi

        case $TLSRet in
         35|51|53|54|58|59|60|64|66|77|80|82|83|90|91)
            tlsLog "CERTERR, S3DnldBlacklist, $TLSRet, $FQDN"
            t2ValNotify "certerr_split" "S3DnldBlacklist, $TLSRet, $FQDN"
            ;;
        esac
        logMessage "Curl return code : $TLSRet"
    fi
...
{    
    ...
    case $TLSRet in
    35|51|53|54|58|59|60|64|66|77|80|82|83|90|91)
        tlsLog "CERTERR, DumpUL, $TLSRet, $FQDN"
        t2ValNotifyc"certerr_split" "DumpUL, $TLSRet, $FQDN"
    ;;
    esac
}
...
uploadToS3()
{
    ...
    if [ "$DEVICE_TYPE" = "hybrid" ] || [ "$DEVICE_TYPE" = "mediaclient" ]; then
       ...
       bootstrapSsrUrl=$(tr181Set Device.DeviceInfo.X_RDKCENTRAL-COM_RFC.Feature.CrashportalEndpoint.URL 2>&1 > /dev/null)
       logMessage "mTlsCrashdumpUpload:$mTlsCrashdumpUpload"
    fi
    if [ "$bootstrapSsrUrl" ]; then
       S3_AMAZON_SIGNING_URL="$bootstrapSsrUrl"
    ...
    if [ -f $EnableOCSPStapling ] || [ -f $EnableOCSP ]; then
       CURL_CMD="curl $CERT -s $TLS --cert-status -o $signed_url_file -w \"${CURL_LOG_OPTION}\" --data-urlencode "filename=\"$updatedfile\""\
       ...
       CURL_CMD="curl $CERT -s $TLS -o $signed_url_file -w \"${CURL_LOG_OPTION}\" --data-urlencode "filename=\"$updatedfile\""\
                                             --data-urlencode "firmwareVersion=$CurrentVersion"\
                                             --data-urlencode "env=$BUILD_TYPE"\
                                             --data-urlencode "model=$modNum"\
                                             --data-urlencode "type=$DUMP_NAME" \
    ...
    if [ $ec -eq 0 ]; then
		...
		if [ "$core_output" = "ARM" ];then
            ...
            if [ -f $EnableOCSPStapling ] || [ -f $EnableOCSP ]; then
             ...
             if [ -f $EnableOCSPStapling ] || [ -f $EnableOCSP ]; then
                CURL_CMD="curl $CERT -v -fgL --cert-status --connect-timeout $CURL_UPLOAD_TIMEOUT $TLS -T \"$updatedfile\" -w \"${CURL_LOG_OPTION}\" $S3_URL"
             else
                CURL_CMD="curl $CERT -v -fgL --connect-timeout $CURL_UPLOAD_TIMEOUT $TLS -T \"$updatedfile\" -w \"${CURL_LOG_OPTION}\" $S3_URL"
             fi
	    fi
    ...
    do
		...
		if [ "${#dumpName}" -ge "135" ]; then
	        ...
	        if [ -f /tmp/set_crash_reboot_flag ];then
		     ...
		     tar -zcvf $tgzFile $dumpName $stbLogFile $ocapLogFile $messagesTxtFile $appStatusLogFile $appLogFile $wpeLogFile $VERSION_FILE $CORE_LOG 2>&1 | logStdout
		else
