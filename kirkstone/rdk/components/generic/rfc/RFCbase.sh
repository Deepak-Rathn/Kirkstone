...
##########################################
####     Get target proposition     ####
##########################################
getTargetProposition()
{
    if [ "$WHOAMI_SUPPORT" == "true" ]; then
        target=$(tr181 Device.DeviceInfo.X_RDKCENTRAL-COM_RFC.Bootstrap.TargetProposition 2>&1)
        echo $target
    fi
}
...
sendHttpRequestToServer()
{
    ...
    if [ "$DEVICE_TYPE" = "broadband" ]; then
        ...
        rfcVideoGetAccountId
        JSONSTR='estbMacAddress='$(getEstbMacAddress)'&firmwareVersion='$(getFWVersion)'&env='$(getBuildType)'&model='$(getModel)'&controllerId='$(getControllerId)'&channelMapId='$(getChannelMapId)'&vodId='$(getVODId)'&partnerId='$rfcPartnerId'&targetProposition='$(getTargetProposition)'&accountId='$rfcAccountId'&experience='$(getExperience)'&version=2'
    else
        rfcVideoGetAccountId
    ...
    if [ "$DEVICE_TYPE" = "broadband" ]; then
        ...
        TLSRet=` exec_curl_mtls "$CURL_ARGS"`
    elif [ "$DEVICE_TYPE" = "mediaclient" ]; then
        rfcLogging "MTLS enabled"
...
if [ "$DEVICE_TYPE" = "broadband" ] && [ "`sysevent get rfc_blob_processing`" != "Completed" ] && [ -f /tmp/rfc_agent_initialized ] ;then
    loop=1
    retry=1
    while [ "$loop" = "1" ]
    do
    ...
    rfcLogging "rfc-agent process not running. Not waiting rfc_blob_processing"
fi
